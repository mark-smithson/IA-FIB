...
def get_init(rovers, petitions, load, nBases, bases, G):
    ...
    j = 0
    for i in bases['almacenes']:
        init += '    (= (subministros-base b'+str(i) + ') ' + str(supplies_lst[j]) + ')\n'
        j += 1
    for i in bases['asentamientos']:
        init += '    (= (subministros-base b'+str(i) + ') 0)\n'
    ...
    for i in range(petitions_supplies):
        ...
        init += '    '+\
            ...
            '(= (prioridad-peticion ' + p + ') ' +str(random.randint(1, 3)) + ')\n'
...
def write_file(rovers, petitions, load, nBases, seed, 
    ...
    metric = '(:metric minimize (+\n'+\
        '    (* (prioridades-totales) ' + str(priorityFactor) + ')\n'+\
        '    (* (combustible-total) ' + str(fuelFactor) + ')\n'+\
        '))'
